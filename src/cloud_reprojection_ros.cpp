...
/**
 * @file cloud_reprojection_ros.cpp
 * @brief 点云重投影 ROS2 节点实现（适配 Ubuntu 22.04 + ROS2 Humble）
 *
 * 订阅 SLAM 点云（/odin1/cloud_slam）和里程计（/odin1/odometry），
 * 使用精确时间同步对齐后，将三维彩色点云通过鱼眼相机模型
 * 重投影到二维图像平面，发布重投影结果图像。
 */
...
/// 返回固定的激光雷达到 IMU 变换矩阵（Til），包含预设的平移偏移量
...
/// 通过编译时 __FILE__ 路径截取，获取 ROS2 包源代码根目录路径
...
/// ROS2 构造函数，加载参数、初始化订阅者/发布者并注册同步回调
...
/// 声明并获取 ROS2 话题参数，从 calib.yaml 加载相机内参和外参，并初始化 CloudReprojector
...
/// ROS2 点云与里程计同步回调，将点云变换到相机坐标系后重投影并发布图像
...
// ==================== ROS2 Main ====================
/// ROS2 主函数：检查重投影开关，等待 calib.yaml 就绪后启动重投影节点
